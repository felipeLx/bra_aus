...
export default function Home() {
  ...
  const steps = [
    ...
    { icon: "✈️", title: t("home.step2Title"), desc: t("home.step2Desc") },
    ...
    <div className="min-h-screen bg-white">
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                {stats.campaigns > 0 && (
                  <span className="text-sm font-medium text-gray-600 bg-white border border-gray-200 px-3 py-1.5 rounded-full shadow-sm">
                    {t("home.campaignsActive", { count: stats.campaigns })}
                  </span>
                )}
                {stats.passengers > 0 && (
                  <span className="text-sm font-medium text-gray-600 bg-white border border-gray-200 px-3 py-1.5 rounded-full shadow-sm">
                    {t("home.seatsReserved", { count: stats.passengers })}
                  </span>
                )}
              </div>
            )}
      ...
      <section className="bg-gray-900 text-white py-20">
        <div className="max-w-5xl mx-auto px-6 text-center">
          <h2 className="text-3xl font-bold mb-3">{t("home.priceTitle")}</h2>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {PRICE_EXAMPLES.map((pax) => (
              <div key={pax} className="bg-white/5 hover:bg-white/10 rounded-2xl p-5 transition-colors">
                <p className="text-gray-400 text-sm mb-1">{pax} {t("home.passengers")}</p>
                <p className="text-2xl font-bold">{formatAud(1_000_000 / pax)}</p>
                <p className="text-gray-500 text-xs mt-1">{t("home.perPerson")}</p>
              </div>
            ))}
          </div>
      ...
      <section className="py-24 bg-white">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-16">
            ...
            <p className="mt-3 text-gray-400">{t("home.howItWorksDesc")}</p>
          </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          <div className="bg-white rounded-3xl p-10 shadow-sm border border-indigo-100 flex flex-col md:flex-row items-start md:items-center gap-8">
            ...
            <div className="flex-1">
              <h2 className="text-2xl font-bold text-gray-900">{t("home.cargoTitle")}</h2>
      ...
      <footer className="border-t border-gray-100 py-8">
        <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-gray-400">
          ...
          <div className="flex gap-5">
            <Link to="/campaigns" className="hover:text-gray-600 transition-colors">{t("nav.campaigns")}</Link>
            {user && <Link to="/dashboard" className="hover:text-gray-600 transition-colors">{t("nav.myTrips")}</Link>}
            {!user && <Link to="/register" className="hover:text-gray-600 transition-colors">{t("home.register")}</Link>}
          </div>
        </div>
      </footer>
    </div>
